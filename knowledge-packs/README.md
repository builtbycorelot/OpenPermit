...
1. **Deterministic** — Same input, same output. No AI interpretation, no probabilistic reasoning.
2. **Testable** — Every rule includes test cases with known-good and known-bad examples.
3. **Versioned** — Semantic versioning tied to specific code editions (IRC 2021, IBC 2021, etc.).
4. **Open Source** — MIT licensed, forkable for local amendments.
5. **Auditable** — Validation logic is public, reviewable code.

### What They Are NOT

- ❌ **Not AI code interpreters** — Rules are written by human experts (code officials, engineers), not generated by LLMs
- ❌ **Not replacements for engineering** — Performance-based designs and complex systems still require PE review
- ❌ **Not discretionary** — Rules check objective requirements only (spacing, dimensions, quantities)
...
knowledge-packs/
├── IRC2021/
│   ├── pack.json                 # Pack manifest (JSON-LD)
│   ├── irc_r602_framing.py       # R602 Wall Framing rules
│   ├── irc_r305_heights.py       # R305 Room Height rules
...
|---------|-------------|---------|-------|--------|
| `IRC2021` | IRC 2021 | 1.0.0 | 3 | ✅ Active |
| `IBC2021` | IBC 2021 | 0.1.0 | 0 | 🚧 In Development |
| `IECC2021` | IECC 2021 | 0.1.0 | 0 | 🚧 In Development |
...
```
IRC2021-austin-tx/
  ├── pack.json              # Extends IRC2021 base
  ├── austin_amendments.py   # Local code amendments
  └── ...
```
...
- ✅ Dimensional requirements (e.g., "not less than 7 feet")
- ✅ Spacing limits (e.g., "not more than 16 inches on center")
- ✅ Quantity requirements (e.g., "one smoke detector per bedroom")
- ✅ Prescriptive tables (e.g., joist span tables)

**Poor candidates (require human judgment):**
- ❌ "Adequate structural capacity" (without specific values)
- ❌ "Reasonable access" (subjective)
- ❌ Performance-based criteria (requires engineering calculation)
...
- **Major (x.0.0)** — Code edition change (IRC 2021 → IRC 2024)
- **Minor (1.x.0)** — New rules added (backward compatible)
- **Patch (1.0.x)** — Bug fixes, clarifications (no new rules)
...
All OpenPermit knowledge packs are **MIT licensed** — free to use, modify, and redistribute.
...
We don't train AI to interpret building codes. We encode objective requirements as deterministic functions — **transparent, testable, and trusted**.
